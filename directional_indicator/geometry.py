...
def calculate_indicator_geometry(
    ...
    """Place an indicator along the direction ray at a fraction of the screen edge.

    ``position_fraction`` controls how far from the screen centre the indicator
    sits: 1.0 = at the inset edge (original behaviour), 0.55 = 55% of the way
    from centre to edge (recommended — visible without blocking the centre).
    ...
    frac   = _finite_number(position_fraction, "position_fraction")
    if radius < 0.0:
    ...
    # Edge intersection, then pull back toward centre by position_fraction.
    ray_scale = min(x_scale, y_scale) * frac
